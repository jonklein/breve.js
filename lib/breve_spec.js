...
(function() {
  ...
  describe('breve.CollisionDetectorBase ', function() {
    beforeEach(function() {
      this.collider = new breve.CollisionDetectorBase();
      return this.array = [5, 7, 9, 2, 3];
    });
    return describe('insertion sort', function() {
      it('should sort objects with default predicate', function() {
        this.collider.insertionSort(this.array);
        return expect(this.array).toEqual([2, 3, 5, 7, 9]);
      });
      it('should use sort predicate', function() {
        this.collider.insertionSort(this.array, function(i, j) {
          return j - i;
        });
        return expect(this.array).toEqual([9, 7, 5, 3, 2]);
      });
      return it('should invoke swap callback for each swapped pair', function() {
        var callbacks,
          _this = this;
        callbacks = {
          swap: function(i, j) {}
        ...
        this.collider.insertionSort(this.array, null, callbacks.swap);
        ...
        return expect(callbacks.swap).toHaveBeenCalledWith(3, 5);
      });
    });
  });
